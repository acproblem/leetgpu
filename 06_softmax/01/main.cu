...
// 2. 计算分母
__global__ void softmax_kernel2(float* input, float* tmp, int N) {
...
// 3. 相除
__global__ void softmax_kernel3(float* input, float* output, float t, int N) {
    int tid = blockDim.x * blockIdx.x + threadIdx.x;

    if (tid < N)
        output[tid] = input[tid] / t;
}
...
void softmax_gpu(float* input, float* output, int N) {
    ...
    float* tmp = nullptr;
    cudaMalloc(&tmp, tmp_size * sizeof(float));
    dim3 block2(BLOCK_SIZE2, 1);
    dim3 grid2(tmp_size, 1);
    softmax_kernel2<<<grid2, block2>>>(input, tmp, N);
    float* h_tmp = new float[tmp_size];
    cudaMemcpy(h_tmp, tmp, tmp_size * sizeof(float), cudaMemcpyDeviceToHost);
    float t = sum_cpu(h_tmp, tmp_size);

    dim3 block3(BLOCK_SIZE3, 1);
    dim3 grid3((N + block3.x - 1)/ block3.x, 1);
    softmax_kernel3<<<grid3, block3>>>(input, output, t, N);
}
...
int main(int argc, char *argv[]) {
    ...
    if (enable_check) {
        ...
        for (int i = 0; i < N; i++) {
            if (fabs(h_output[i] - output_cpu[i]) > 1e-5) {
