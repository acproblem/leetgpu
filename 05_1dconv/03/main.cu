...
__global__ void convolution_1d_kernel(const float* input, float* output, int input_size, int kernel_size) {
    ...
    // 将 input 搬运到 sInput
    for (int step = 0; threadIdx.x + step < blockDim.x + kernel_size && tid + step < input_size; step+=blockDim.x) {
        sInput[threadIdx.x + step] = input[tid + step];
    }
    __syncthreads();
    ...
    if (tid <= input_size - kernel_size) {
        for (int j = 0; j < kernel_size; j++) {
            tmp += sInput[threadIdx.x + j] * kernel[j];
        }
        output[tid] = tmp;
    }
}
...
int main(int argc, char *argv[]) {
    ...
    CHECK(cudaMemcpyToSymbol(kernel, h_kernel, kernel_size * sizeof(float)));
    ...
    for (int i = 0; i < 10; ++i)
        convolution_1d_kernel<<<grid, block, (block.x + kernel_size) * sizeof(float)>>>(d_input, d_output, input_size, kernel_size);
