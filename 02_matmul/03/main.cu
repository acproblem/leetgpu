...
__global__ void matmul(const float* A, const float* B, float* C, int M, int N, int K) {
    ...
    for (int k = 0; k < K; k += BK) {
        // 读取A分块到SA
        for (int t = 0; t < TM; ++t) {
            int gARow = blockIdx.y * BM + threadIdx.y * TM + t;
            int gACol = k + threadIdx.x;
            if (gARow < M && gACol < K)
                SA[threadIdx.y * TM + t][threadIdx.x] = A[gARow * K + gACol];
            else
                SA[threadIdx.y * TM + t][threadIdx.x] = 0.0f;
        }
        // 读取B分块到SB
        for (int t = 0; t < TN; ++t) {
            int gBRow = k + threadIdx.y;
            int gBCol = blockIdx.x * BN + threadIdx.x * TN + t;
            if (gBRow < K && gBCol < N) 
                SB[threadIdx.y][threadIdx.x * TN + t] = B[gBRow * N + gBCol];
            else
                SB[threadIdx.y][threadIdx.x * TN + t] = 0.0f;
        }
        ...
        for (int k = 0; k < BK; ++k) {
            for (int i = 0; i < TM; ++i) {
                int sARow = threadIdx.y * TM + i;
                for (int j = 0; j < TN; ++j) {
                    int sBCol = threadIdx.x * TN + j;
                    res[i][j] += SA[sARow][k] * SB[k][sBCol];
                }
            }
        }
    ...
    // 写入 C[row][col]
    for (int i = 0; i < TM; ++i) {
        int gCRow = blockIdx.y * BM + threadIdx.y * TM + i;
        for (int j = 0; j < TN; ++j) {
            int gCCol = blockIdx.x * BN + threadIdx.x * TN + j;
            if (gCRow < M && gCCol < N)
