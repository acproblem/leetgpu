...
__global__ void matmul(const float* A, const float* B, float* C, int M, int N, int K) {
    ...
    for (int k = 0; k < K; k += BK) {
        ...
        for (int t = 0; t < TM; ++t) {
            ...
            if (gARow < M && gACol < K)
                SA[threadIdx.y + STRIDE_M * t][threadIdx.x] = A[gARow * K + gACol];
            else
                SA[threadIdx.y + STRIDE_M * t][threadIdx.x] = 0.0f;
        }
        // 读取B分块到SB
        for (int t = 0; t < TN; ++t) {
            ...
            if (gBRow < K && gBCol < N) 
                SB[threadIdx.y][threadIdx.x + STRIDE_N * t] = B[gBRow * N + gBCol];
            else
                SB[threadIdx.y][threadIdx.x + STRIDE_N * t] = 0.0f;
        }
