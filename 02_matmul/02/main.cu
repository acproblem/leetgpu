...
__global__ void matmul(const float* A, const float* B, float* C, int M, int N, int K) {
    float res = 0.0f;  // 计算 C[row][col] 的寄存器
    ...
    for (int k = 0; k < K; k += BK) {
        // 读取数据到共享内存
        int gARow = blockIdx.y * BM + threadIdx.y;
        int gACol = k + threadIdx.x;
        int gBRow = k + threadIdx.y;
        int gBCol = blockIdx.x * BN + threadIdx.x;
        if (gARow < M && gACol < K)
            SA[threadIdx.y][threadIdx.x] = A[gARow * K + gACol];
        else
            SA[threadIdx.y][threadIdx.x] = 0.0f;
        if (gBRow < K && gBCol < N)
            SB[threadIdx.y][threadIdx.x] = B[gBRow * N + gBCol];
        else
        ...
        // 计算
        for (int k = 0; k < BK; ++k) {
            res += SA[threadIdx.y][k] * SB[k][threadIdx.x];
        }
    ...
    // 写入 C[row][col]
    int gCRow = threadIdx.y + blockIdx.y * blockDim.y;
    int gCCol = threadIdx.x + blockIdx.x * blockDim.x;
    if (gCRow < M && gCCol < N)
        C[gCRow * N + gCCol] = res;
}
